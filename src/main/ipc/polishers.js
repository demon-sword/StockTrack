...
ipcMain.handle(`${TABLE}-create`, async (_, data) => {
  const result = await run(
    `INSERT INTO ${TABLE} (name, contact, phone, payment_rate_per_piece) VALUES (?, ?, ?, ?)`,
    [data.name, data.contact || '', data.phone || '', data.payment_rate_per_piece || 0]
  );
...
ipcMain.handle(`${TABLE}-update`, async (_, id, data) => {
  await run(
    `UPDATE ${TABLE} SET name = ?, contact = ?, phone = ?, payment_rate_per_piece = ? WHERE id = ?`,
    [data.name, data.contact || '', data.phone || '', data.payment_rate_per_piece || 0, id]
  );
  return { id, ...data };
});
